...
class Trade:
    ...
    def look_for_all_positives(self):
        symbol = "BTCEUR"
        self.set_use_volume(False)
        for ema1 in range(1,11):
            for ema2 in range(10,51):
                for ema3 in range(100,201):
                    all_profit = []
                    profit = []
                    for month in range(1,13):
                        value = float(self.backtest_cross_ema_strategy(symbol, str(month).zfill(2) + "-2023", 30, low=ema1, high=ema2, long=ema3))
                        # print(ema1, ema2, ema3, month, value)  
                        if value < 0:
                            profit = None
                            break
                        profit.append(value)
                    if profit is not None:
                        all_profit.append({
                            ...
                            "ema3": ema3,
                            "value": profit,
                            "avg": round(float(np.mean(profit)),2)
                        })
                print(all_profit)
            print(all_profit)
        print(all_profit)
...
if __name__ == "__main__":
    ...
    for month in range(1,13):
        ...
        for use_volume in [False, True]:
            ...
            value = float(trade.backtest_cross_ema_strategy(symbol, str(month).zfill(2) + "-2023", 30, low=9))
            a.append(value)
            if use_volume:
                yes_volume.append(value)
            else:
                no_volume.append(value)
        print(a)
    print(np.mean(no_volume), np.mean(yes_volume))
